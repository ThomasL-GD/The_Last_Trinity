...
public class AutoRotation : MonoBehaviour {
    ...
    void Update()
    {
        //If we previously had no target but now we have one
        ...
        if (!m_targetNull) {
            ...
            Vector3 positionTarget = m_target.position;
            ...
            if (m_durationLeft < 0 || (Mathf.Abs((positionTarget + m_offsetTarget - transform.position).magnitude) <= m_uncertainty)) {
                m_target = null;
                m_targetNull = true;
                m_visualEffect.Stop();
            }
